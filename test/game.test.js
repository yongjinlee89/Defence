...
/* ---------------- 건설/증설/철거/경제 ---------------- */
{
  ...
  // 수입: 땅 기본 0.6×3 + 공장 2개 × 등급 3 × 1 = 7.8/초 (수도는 최고 등급)
  assert.strictEqual(ca.yield, 3);
  assert.ok(Math.abs(g.incomeOf(p) - 7.8) < 1e-9);
  const cash0 = p.cash;
  tick(g, 10);
  // 수입 80 − 보병 8명 유지비 (8×0.1×10 = 8)
  assert.ok(Math.abs(p.cash - (cash0 + 78 - 8)) < 0.01, '10초에 78 − 유지비 8');
  assert.ok(Math.abs(g.upkeepOf(p) - 0.8) < 1e-9);
  // 돈이 바닥나면 탈영
  p.cash = 0;
  ca.units.inf = 200; // 유지비 20/초 > 수입 7.8/초
  tick(g, 10);
  ...
  assert.ok(Math.abs(g.incomeOf(p) - 10.8) < 1e-9);
  ...
  assert.ok(Math.abs(g.incomeOf(p) - (inc0 + 1.6)) < 1e-9, '기본 0.6 + 공장 1 만큼 오른다');
...
/* ---------------- 공개 상태 ---------------- */
{
  ...
  assert.strictEqual(s.players[0].income, 4.8);
